...
public static class ScaleOscillatingIconPatch
{
    ...
    private static IEnumerable<CodeInstruction> ScaleOscillatingIcon(IEnumerable<CodeInstruction> instructions)
    {
        ...
        return ScaleHelper.ScaleVector2Identities(instructions, ScaleHelper.IconScalePropertyGetter!);
    }
}
...
public static class ScaleSelectedIconPatch
{
    ...
    {
        return ScaleHelper.ScaleVector2Identities(instructions, ScaleHelper.IconScalePropertyGetter!, numInstancesToEdit: 1); // Only scale the icon container, not the icons as well
    }
}
